...
int main(int argc, char** argv)
{
    ...
    FILE* fileHandler = fopen("kernel_mirror.cl", "r");
    if (fileHandler == NULL) {
    ...
    cl_kernel kernel = clCreateKernel(program, "image_mirror", &err);
    cl_error(err, "Failed to create kernel from the program\n");

    // 5. Create input and output arrays

    CImg<unsigned char> img("Pilar_Zaragoza.jpg");

	unsigned int n_rows = img.height();
	unsigned int n_cols = img.width();
	size_t size = n_rows * n_cols * 3;
    unsigned char imagen[size];
    int pos_img = 0;

	for (int cols = 0; cols < n_cols; cols++) {
		for (int rows = 0; rows < n_rows; rows++){
			imagen[pos_img] = img(cols, rows, 0, 0); //R
			pos_img++;

			imagen[pos_img] = img(cols, rows, 0, 1); //G
			pos_img++;

			imagen[pos_img] = img(cols, rows, 0, 2); //B
			pos_img++;
		}
	}
	
    // 6. Create OpenCL buffer visible to the OpenCl runtime
    cl_mem in_device_object = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_char) * size, NULL, &err);
    cl_error(err, "Failed to create memory buffer at device\n");

    // 7. Write date into the memory object
    auto start_commandqueue_hostDevice = std::chrono::high_resolution_clock::now();
    err = clEnqueueWriteBuffer(command_queue, in_device_object, CL_TRUE, 0, sizeof(unsigned char) * size, imagen, 0, NULL, NULL);
	...
	err = clSetKernelArg(kernel, 1, sizeof(n_rows), &n_rows);
    cl_error(err, "Failed to set argument 1\n");
	err = clSetKernelArg(kernel, 2, sizeof(n_cols), &n_cols);
    cl_error(err, "Failed to set argument 1\n");
    ...
    err = clEnqueueReadBuffer(command_queue, in_device_object, CL_TRUE, 0, sizeof(unsigned char) * size, imagen, 0, NULL, NULL);
	...
	for(int cols = 0; cols < n_cols; cols++){
		for (int rows = 0; rows < n_rows; rows++) {
			img(cols, rows, 0, 0) = imagen[pos_img]; //R
			pos_img++;

			img(cols, rows, 0, 1) = imagen[pos_img]; //G
			pos_img++;

			img(cols, rows, 0, 2) = imagen[pos_img]; //B
			pos_img++;
		}
    }

    //img.display("image.jpg"); // Display the image in a display window
    img.save("Pilar_fliped.jpg");
    ...
    std::cout << "\nAll memory objects released with success.\n" << std::endl;
    ...
    std::cout << "The memory footprint is: " << memory_footPrint << " bytes\n" << std::endl;
    std::cout << "Bandwith host-device -> sizeof(cl_char) * size / time needed to pass the data: " << sizeof(cl_char) * size / commandqueue_duration_hostDevice.count() << " bytes/second\n";
    std::cout << "Bandwith device-host -> sizeof(cl_char) * size / time needed to pass the data: " << sizeof(cl_char) * size / commandqueue_duration_deviceHost.count() << " bytes/second\n" << std::endl;
    std::cout << "Throughput of the kernel: " << img.width()*img.height() / (nanoSeconds / 1000000000.0) << " pixels / second" << std::endl;
    std::cout << "Kernel does 6 loadings and 6 storages each time it is launched, " <<  
	"so the kernel bandwith would be: " << 12*sizeof(cl_uchar)*size/6 / (nanoSeconds / 1000000000.0) << " bytes / second\n" << std::endl;
